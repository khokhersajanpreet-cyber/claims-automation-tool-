...
def analyze_pendency_and_date(row, pendency_col, exp_date_col, work_date_col, remarks_col):
    msg = str(row[remarks_col]).lower()
    if msg == "agent not shared remarks on whatsapp":
        return row[pendency_col], row[exp_date_col]
    ...
    exp_date = row[exp_date_col]
    work_date = pd.to_datetime(row[work_date_col], errors='coerce')
...
if st.button("🚀 Process and Auto-Fill Pendency Sheet", type="primary"):
    if not chat_file or not pendency_file:
        ...
        with st.spinner("Processing chat logs and updating pendency rows..."):
            ...
            df_chat['DateTime_str'] = df_chat['Date'].astype(str) + ' ' + df_chat['Time'].astype(str).str.replace(r'[\u202f\u200e\u202a\u202c\u202d\u2069]', ' ', regex=True)
            ...
            lead_col = [c for c in df_pendency.columns if 'lead' in c.lower()][0]
            remarks_col = [c for c in df_pendency.columns if 'remark' in c.lower()][0]
            pendency_col = [c for c in df_pendency.columns if 'pendency' in c.lower()][0]
            exp_date_col = [c for c in df_pendency.columns if 'expected' in c.lower()][0]
            work_date_col = [c for c in df_pendency.columns if 'work date' in c.lower()][0]
